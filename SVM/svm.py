...
def weight_vector(x, y, alpha):
    ...
    for i in range(len(x)):
        term=[xx*alpha[i]*y[i] for xx in x[i]]
        w=[tt+ww for tt,ww in zip(term,w)]
...
def find_support(x, y, w, b, tolerance=0.001):
    ...
    for i in range(length):
        hyperPlane=sum(xx*ww for xx,ww in zip(x[i], w))
        hyperPlane+=b
        if y[i] == 1:
            if abs(hyperPlane - 1 ) < tolerance:
                support.add(i)
        elif y[i] == -1:
            if abs(hyperPlane + 1) < tolerance:
                support.add(i) 
...
def find_slack(x, y, w, b):
    ...
    for i in range(length):
        hyperPlane=sum(xx*ww for xx,ww in zip(x[i], w))
        hyperPlane+=b
#         print hyperPlane,y[i]
        if y[i] == 1 and (hyperPlane < 0):
            slack.add(i)
        elif y[i] == -1 and (hyperPlane > 0):
            slack.add(i)
