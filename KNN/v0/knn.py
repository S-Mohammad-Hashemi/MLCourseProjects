...
class Knearest:
    ...
    def testMajority(self,item_indices):
        """
...
#         res=bincount(mylist)
        res=itemfreq(mylist)
#         print("###############",res)
        temp=res.argmax(axis=0)
#         biggest_index=argmax(res)
        biggest_index=temp[1]
#         print("BIggest index IS: ",res[biggest_index][0])
#         biggest=res[biggest_index]
        biggest=res[biggest_index][0]
#         print("biggest index is: ",biggest_index,biggest)
        res[biggest_index][0]=0;
#         sec_biggest_index=argmax(res)
        temp=res.argmax(axis=0)
        sec_biggest_index=temp[1]
        sec_biggest=res[sec_biggest_index][0]
#         print("sec biggest index is: ",sec_biggest_index,sec_biggest)
        res=-1
        if(biggest_index==sec_biggest_index):
#             print("go to median")
    ...
    def majority(self, item_indices):
        ...
        for data in mylist:
            if(data<0):
                ...
                break
        if(test_flag==True):
            return self.testMajority(mylist)
        res=bincount(mylist)
        biggest_index=argmax(res)
        biggest=res[biggest_index]
#         print("biggest index is: ",biggest_index,biggest)
...
#         print("index_array is:",index_array[0])
        #### I assume tha if mylist was [1,1,2,3,3] the classify function should return 2 because of median!
