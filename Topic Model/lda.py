...
class VariationalBayes:
    ...
    def new_phi(gamma, beta, word, count):
        ...
        for i in range(len(gamma)):
            phi=math.exp(digam(gamma[i])-digam(sum_gamma))*beta[i][word]
            total+=phi
            phi_list.append(phi)
        for i in range(len(phi_list)):
            phi_list[i]=phi_list[i]/total
        for i in range(len(phi_list)):
            phi_list[i]=phi_list[i]*count
        return phi_list
...
#         return ones(len(gamma)) / float(len(gamma))
    ...
    def m_step(self, topic_counts):
        ...
        for i in range(len(self._beta)):
            sum_of_row=sum(topic_counts[i])
            for j in range(len(self._beta[0])):
                self._beta[i][j]=topic_counts[i][j]/sum_of_row
